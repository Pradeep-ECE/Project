...
models.sequelize.authenticate().then(()=>{
  console.log('connected to pgAdmin database:sequelize');
}).catch((err) => {
  console.log('unable to connect to pgAdmin database:sequelize',err.message);
});
models.sequelize.sync({alter:true});
app.use('/employee',require('./controllers/employeeController/employee.Controller').router);
app.use('/settings',require('./controllers/settingsController/settings.Controller').router);

// catch 404 and forward to error handler
app.use(function(req, res, next) {
  next(createError(404));
});
// jobService.sendBirthdayWish();
// error handler
